...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
  ...
  IconData _getSpecIcon(String key) {
    ...
    if (lowerKey.contains('vol') ||
        lowerKey.contains('isi') ||
        lowerKey.contains('ukuran')) {
      return Icons.opacity_rounded;
    } else if (lowerKey.contains('kemas') || lowerKey.contains('wadah')) {
      return Icons.inventory_2_rounded;
    } else if (lowerKey.contains('kadal') ||
        lowerKey.contains('expired') ||
        lowerKey.contains('exp') ||
        lowerKey.contains('waktu') ||
        lowerKey.contains('simpan')) {
      return Icons.calendar_today_rounded;
    } else if (lowerKey.contains('arom') ||
        lowerKey.contains('bau') ||
        lowerKey.contains('wangi')) {
  ...
  void _addToCart() {
    ...
    if (currentQty + _qty > widget.product.stock) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Gagal menambahkan. Total di keranjang melebihi stok (${widget.product.stock} item)',
          ),
                              ...
                              children: [
                                ...
                                Column(
                                  children: List.generate(
                                    ...
                                    (index) {
                                      final i = index * 2;
                                      final key1 = activeSpecs.keys.elementAt(
                                        i,
                                      );
                                      final val1 =
                                          activeSpecs[key1]?.toString() ?? '';

                                      final hasSecond =
                                          i + 1 < activeSpecs.length;
                                      final key2 = hasSecond
                                          ? activeSpecs.keys.elementAt(i + 1)
                                          : null;
                                      final val2 = hasSecond
                                          ? activeSpecs[key2]?.toString() ?? ''
                                          : '';
                              ...
                              Expanded(
                                child: Column(
                                  children: List.generate(5, (i) {
                                    final star = 5 - i;
                                    final count = _fetchedReviews
                                        .where((r) => r.rating == star)
                                        .length;
                                    final pct = _fetchedReviews.isEmpty
                                        ? 0.0
                                        : count / _fetchedReviews.length;
                                    return Padding(
                                      padding: const EdgeInsets.symmetric(
                                        vertical: 2,
                                      ),
